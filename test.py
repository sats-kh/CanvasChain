...
def generate_inpainted_image(inpaint_data, prompt, negative_prompt, strength, guidance_scale):
    ...
    # 4. mask_layer를 white_bg 위에 붙여넣기 (mask_layer 자체를 투명도로 사용)
    white_bg.paste(mask_layer, (0, 0), mask_layer)
    # 5. 그 결과를 그레이스케일(L)로 변환
    mask_image = white_bg.convert("L")
    # 6. 최종적으로 반전: 원래 brush 영역(어두운 부분)은 흰색, 배경(밝은 부분)은 검은색이 되도록 처리
    mask_image = Image.eval(mask_image, lambda x: 255 - x)
    # 7. 임계값 처리: 128 이상의 값은 255, 미만은 0으로 변환
    mask_image = mask_image.point(lambda p: 255 if p > 128 else 0)
    
    result = inpaint2img_pipe(
        prompt=prompt,
        image=bg,
        ...
        negative_prompt=negative_prompt,
        num_inference_steps=50  # 원하는 스텝 수로 조정
    ).images
    return result[0]
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            inpaint2img_btn = gr.Button("Generate Image-to-Image")
